...
class RedisService {
    ...
    // 알림 기록 삭제
    async clearNotificationSent(busId, stopId) {
        const key = `notification:${busId}:${stopId}`;
    ...
    async getAllActiveBuses() {
        ...
        const promises = keys.map(async (key) => {
            ...
            if (value) {
                try {
                    ...
                    logger_1.default.error(`Error parsing bus location from key ${key}:`, error);
                    return null;
                }
            }
            return null;
        ...
        logger_1.default.debug(`Retrieved ${buses.length} active buses`);
        return buses;
    }
    // 호출 취소 (대규모 시스템용)
    async cancelCall(callId) {
        ...
        if (parts.length >= 3) {
            ...
            const key = `call:${stopId}:${routeId}`;
            await this.client.del(key);
            logger_1.default.info(`Call cancelled: ${callId}`);
        }
    }
    // 헬스 체크
    isHealthy() {

...
class ETAProcessorService {
    ...
    busStopRegions = new Map();
    // 활성 버스 캐시 (성능 최적화)
    activeBusCache = new Map();
    busLocationCache = new Map();
    cacheUpdateTime = 0;
    CACHE_TTL_MS = 5000; // 5초 캐시
    constructor() {
    ...
    initializeRegionalData() {
        ...
        logger_1.default.info(`Initialized ${regions.length} regions with ${totalStops} bus stops`);
    }
    generateBusStopsForRegion(regionName, centerLat, centerLng, count) {
        const stops = [];
    ...
    async startProcessing() {
        ...
        logger_1.default.info('Starting large-scale ETA processing...');
        logger_1.default.info(`Total regions: ${this.busStopRegions.size}`);
    ...
    async processETACalculations() {
        ...
        try {
            ...
            // 병렬로 모든 지역 처리
            const regionPromises = Array.from(this.busStopRegions.values()).map(region => this.processRegionETAs(region));
            ...
            logger_1.default.info(`ETA processing completed: ${this.processingStats.activeCallsProcessed} calls, ` +
                `${this.processingStats.etaCalculationsPerformed} calculations in ${this.processingStats.processingTimeMs}ms`);
        }
        catch (error) {
            logger_1.default.error('Error in large-scale ETA processing:', error);
        }
        finally {
            this.isProcessing = false;
        }
    }
    async updateActiveBusCache() {
        const now = Date.now();
        // 캐시가 유효하면 업데이트 스프리트
        ...
        try {
            // 모든 활성 버스 위치 조회
            const allBuses = await redis_1.default.getAllActiveBuses();
            // 버스 위치 캐시 업데이트
            ...
            logger_1.default.debug(`Bus cache updated: ${allBuses.length} active buses across ${this.activeBusCache.size} routes`);
        }
        catch (error) {
            logger_1.default.error('Error updating bus cache:', error);
        }
    }
    async processRegionETAs(region) {
        try {
            // 지역 내 모든 정류장을 병렬로 처리
            ...
            logger_1.default.error(`Error processing region ${region.regionId}:`, error);
        }
    }
    async processStopETAs(stopId, busStop) {
        try {
            ...
            logger_1.default.error(`Error processing stop ${stopId}:`, error);
        }
    }
    async processCallETA(call, busStop) {
        try {
            ...
            if (buses.length === 0) {
                logger_1.default.debug(`No active buses found for route ${call.routeId}`);
                return;
            }
            // 정류장과 가까운 버스들만 필터링 (10km 이내)
            const nearbyBuses = buses.filter(bus => {
                const distance = location_1.default.calculateDistance({ latitude: bus.latitude, longitude: bus.longitude }, { latitude: busStop.latitude, longitude: busStop.longitude });
            ...
            // 가장 가까운 버스 찾기
            const closest = results.reduce((closest, current) => !closest || current.eta.distanceMeters < closest.eta.distanceMeters ? current : closest);
            if (closest) {
    ...
    async handleBusPassed(bus, call, busStop) {
        // 버튼 호출 해제
        await redis_1.default.cancelCall(call.id);
        // 버스에게 LED 끄기 알림
        const notification = {
            busId: bus.busId,
            stopId: busStop.stopId,
        ...
        await mqtt_1.default.sendNotificationToBus(bus.busId, notification);
        await redis_1.default.clearNotificationSent(bus.busId, busStop.stopId);
        logger_1.default.info(`Bus ${bus.busId} passed stop ${busStop.stopId}, call cancelled`);
    }
    ...
    async processImmediateETAForBus(busLocation) {
        // 버스 위치 기준으로 근처 지역 찾기
    ...
    findNearbyRegions(latitude, longitude, radiusKm) {
        const nearbyRegions = [];
        for (const region of this.busStopRegions.values()) {
            const distance = location_1.default.calculateDistance({ latitude, longitude }, { latitude: region.centerLat, longitude: region.centerLng }) / 1000; // 미터를 킬로미터로 변환
    ...
    // 모든 정류장 정보 조회 (페이징 지원)
    getAllBusStops(page = 1, limit = 50) {
        const allStops = [];
        for (const region of this.busStopRegions.values()) {
